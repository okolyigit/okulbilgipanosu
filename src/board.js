...
function extractYouTubeId(url) {
  if (!url) return null;
  // youtube.com/watch?v=ID veya youtube.com/watch?...&v=ID
  let m = url.match(/[?&]v=([a-zA-Z0-9_-]{11})/);
  if (m) return m[1];
  // youtu.be/ID
  m = url.match(/youtu\.be\/([a-zA-Z0-9_-]{11})/);
  if (m) return m[1];
  // youtube.com/embed/ID veya /shorts/ID
...
function buildYouTubeEmbedUrl(videoId) {
  const params = new URLSearchParams({
    ...
    controls: '0',
    loop: '1',
    playlist: videoId,         // loop=1 için zorunlu
    modestbranding: '1',
    rel: '0',
    playsinline: '1',
    iv_load_policy: '3',
...
function renderSlider(announcements) {
  ...
  items.forEach((item, i) => {
    ...
    if (videoId) {
      // Video slaytı — iframe, yazı yok
      slide.classList.add('slide-video');
      // Lazy: sadece active olunca yükle (multi-slide'da bandwidth)
  ...
  let current = 0;
  function goTo(i) {
    // Eski slaytın iframe'i varsa yükünü düşür (gereksiz oynatmayı durdur)
    ...
    // Yeni video slaytı için iframe'i geç yükle
...
// DB day_of_week: 1=Pzt, ..., 5=Cuma, NULL=her gün
